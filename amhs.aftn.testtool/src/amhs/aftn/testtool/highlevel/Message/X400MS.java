...
public class X400MS {
    ...
    private static Session open(Account account) {
        Out.print("Open connection to " + account.getName());
    ...
    public static NormalMessage getMessage(String account, int timeout) throws X400APIException {
        Out.print("Load account ");
        Account acc = (Account) XmlSerializer.deserialize(account, Account.class);
        Out.print(true);
        ...
        Out.print("Getting AMHS message ");
        int status;
        int seqn = 0;
    ...
    public static void deleteMessage(Session session) {
        ...
        // Report report = null;
         while (true) {
            status = com.isode.x400api.X400ms.x400_ms_msggetstartex(session, seqn, 0, message);
            if (status == X400_att.X400_E_NO_MESSAGE) {
                break;
            }
        ...
        while (true) {
            status = com.isode.x400api.X400ms.x400_ms_msggetstartex(session, seqn, 1, message);
            if (status == X400_att.X400_E_NO_MESSAGE) {
                break;
            }
        ...
        while (true) {
            status = com.isode.x400api.X400ms.x400_ms_msggetstartex(session, seqn, 2, message);
            if (status == X400_att.X400_E_NO_MESSAGE) {
                break;
            }
    ...
    public static void deleteMessage(String account) {
        Out.print("Load account " + account);
        Account acc = (Account) XmlSerializer.deserialize(account, Account.class);
        Out.print(true);
